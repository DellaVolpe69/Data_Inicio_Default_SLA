...
st.set_page_config(
    page_title="Data Início Default SLA",
    layout="wide",
...
# try:
#     df_filial = load_cv_filial()
# except Exception as e:
#     st.error(f"Erro ao carregar CV_FILIAL: {e}")
#     st.stop()

#st.dataframe(df_filial.head())
###################################
...
def carregar_dados():
    data = supabase.table("Data_Inicio_Default_SLA").select("*").execute()
    return pd.DataFrame(data.data)

# Função para inserir
def inserir_registro(
    cliente, tp_medicao, referencia_medicao, categoria, incoterms,
    prioridade, horario_corte, tipo_carga, referencia, data,
    dia_util_inicio, ajuste_dia_inicio, ajuste_horario_inicio,
    dia_util_entrega, ajuste_dia_entrega, ajuste_horario_entrega,
    ajuste_dia_coleta, ref_cid_destino, vigencia_inicio
):

    # -----------------------------
    # 🚀 Enviar ao Supabase
    # -----------------------------
    supabase.table("Data_Inicio_Default_SLA").insert({
        "CLIENTE": cliente,
        ...
        "DATA": data,
        "DIA_UTIL_INICIO": dia_util_inicio,
        "AJUSTE_DIA_INICIO": ajuste_dia_inicio,
        "AJUSTE_HORARIO_INICIO": ajuste_horario_inicio,
        "DIA_UTIL_ENTREGA": dia_util_entrega,
        "AJUSTE_DIA_ENTREGA": ajuste_dia_entrega,
        "AJUSTE_HORARIO_ENTREGA": ajuste_horario_entrega,
        "AJUSTE_DIA_COLETA": ajuste_dia_coleta,
        "REF_CID_DESTINO": ref_cid_destino,
...
def atualizar_registro(id,
    ...
    supabase.table("Data_Inicio_Default_SLA").update({
        "REFERENCIA_MEDICAO": referencia_medicao,
        "INCOTERMS": incoterms,
        "PRIORIDADE": prioridade,
        "TIPO_CARGA": tipo_carga,
...
def excluir_registro(id):
    supabase.table("Data_Inicio_Default_SLA").delete().eq("ID", id).execute()
...
def verificar_existencia(referencia_medicao, incoterms, prioridade, tipo_carga, vigencia_inicio):
    result = (
        supabase.table("Data_Inicio_Default_SLA")
        .select("ID")
...
if st.session_state.pagina == "Cadastrar":
    ...
    "📝 Cadastrar Data Início Default SLA"
    "</h1>",
    unsafe_allow_html=True
)
    ...
    categoria = st.selectbox(
        "Categoria",
        options=["MERCADORIA INDISPONIVEL", "NAO SE APLICA"],
        index=0  
    )
    ...
    prioridade = st.selectbox(
        "Prioridade",
        options=["NORMAL", "EXPRESSA"],
        index=0  
    )
    
    ############################################
    
    horario_corte = st.selectbox(
        "Horário de Corte",
        options=["NAO SE APLICA", "11", "12", "13", "14", "15", "16", "17"],
        index=0  
    )
    ...
    tipo_carga = st.selectbox(
        "Tipo de Carga",
        options=["FRACIONADO", "LOTACAO", "DEDICADO", "PRODUTO QUIMICO"],
        index=0  
    )
    ...
    referencia = st.selectbox(
        "Referência",
        options=["MAIOR DATA", "ENTRADA DA NOTA", "EMISSAO DO CTE", "DATA PARAMETRIZADA"],
        index=0  
    )
    ...
    data = st.selectbox(
        "Data",
        options=["", "DATA_ALVO", "DATA_SOLIC_NIMBI", "MAIOR PROG x SOLIC + 2", "ULTIMO_ACEITE"],
        index=0  
    )
    ...
    dia_util_inicio = st.selectbox(
        "Dia útil Início",
        options=["[1, 2 ,3 ,4 ,5 ,6, 7]", "[2 ,3 ,4 ,5 ,6]"],
        index=0  
    )
    
    ############################################
    
    ajuste_dia_inicio = st.selectbox(
        "Ajuste Dia Início",
        options=["0", "1"],
        index=0  
    )
    ...
    ajuste_horario_inicio = st.selectbox(
        "Ajuste Horário Início",
        options=["7", "8"],
        index=0  
    )
    ...
    ajuste_dia_entrega = st.selectbox(
        "Ajuste Dia Entrega",
        options=["0", "1"],
        index=0  
    )
    ...
    ajuste_horario_entrega = st.text_input("Ajuste Horário Entrega")
    
    ############################################
    
    ajuste_dia_coleta = st.text_input("Ajuste Dia Coleta")
    
    ############################################
    
    ref_cid_destino = st.selectbox(
        "Referência Cidade Destino",
        options=["DESTINATARIO", "PRESTACAO"],
        index=0  
    )
        ...
        with col2:
            ...
            if st.button("💾 Salvar", use_container_width=True):
                ...
                if existe:                    
                    ...
                    if referencia_medicao and incoterms and prioridade and tipo_carga and vigencia_inicio:
                        inserir_registro(
                            cliente=cliente,
...
# --- PÁGINA EDITAR ---
    ...
    if not df.empty:
        ...
        with st.expander("🔎 Filtros"):
            ...
            with col3:
                filtro_prioridade = st.selectbox(
                    "Prioridade", 
                    ["Todas"] + sorted(df["PRIORIDADE"].unique().tolist())
                )
                
            with col4:
                filtro_tipo_carga = st.selectbox(
                    "Tipo Carga", 
                    ["Todas"] + sorted(df["TIPO_CARGA"].unique().tolist())
                )
        ...
        if filtro_tipo_carga != "Todas":
            df = df[df["TIPO_CARGA"] == filtro_tipo_carga]
        ...
        df.sort_values(by=["ID", "REFERENCIA_MEDICAO", "INCOTERMS", "PRIORIDADE", "TIPO_CARGA", "VIGENCIA_INICIO"], ascending=[False, False, True, True, True, True], inplace=True)
    ...
    if not df.empty:
        ...
        with st.expander("✏️ Editar Registro"):
            ...
            novo_referencia_medicao = st.text_input("REFERENCIA_MEDICAO", registro["REFERENCIA_MEDICAO"])
            ...
            novo_prioridade = st.selectbox(
                "Prioridade",
                options=opcoes_prioridade,
                index=opcoes_prioridade.index(registro["PRIORIDADE"]) 
                    if registro["PRIORIDADE"] in opcoes_prioridade else 0
            )

            novo_tipo_carga = st.selectbox(
                "Tipo Carga",
                options=opcoes_tipo_carga,
                index=opcoes_tipo_carga.index(registro["TIPO_CARGA"]) 
                    if registro["TIPO_CARGA"] in opcoes_tipo_carga else 0
            )
            ...
            if st.button("Salvar Alterações"):
                ...
                if existe:                    
                    ...
                    atualizar_registro(id_registro, novo_referencia_medicao, novo_incoterms, novo_prioridade, novo_tipo_carga, novo_inicio)
